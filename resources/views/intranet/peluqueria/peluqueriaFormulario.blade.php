    ...
    <div class="d-flex justify-content-center" style="min-height: 80vh;">
        <div class="card shadow mt-5" style="width: 100%; max-width: 500px;">
            ...
            <div class="card-body">
                ...
                {{-- Formulario --}}
                <form
                    action="{{ $mode === 'create' ? route('peluqueria.store') : route('peluqueria.update', $peluqueria->id_peluqueria) }}"
                    method="POST">
                    @csrf
                    @if ($mode === 'update')
                        @method('POST')
                    ...
                    <div class="mb-3">
                        <label for="id_cliente" class="form-label">Fecha y hora de corte <span class="text-danger">*</span></label>
                        <input type="datetime-local" id="hora_corte" name="hora_corte" class="form-control"
                            value="{{ old('hora_corte', $peluqueria?->hora_corte?->format('Y-m-d\TH:i')) }}" required>
                    </div>
                    ...
                    <div class="d-grid">
                        <button type="submit"
                            class="btn btn-success">{{ $mode === 'create' ? 'Registrar' : 'Actualizar' }}
                            Peluquería</button>
                    </div>
                </form>
            </div>
